...
void setup() {
  ...
  Serial.println("ESP32: Initialized - Strip 1 (AIR QUALITY DISPLAY), Strip 2 (CLOCK)");
...
void loop() {
  ...
  if (currentTime - lastUpdate >= DISPLAY_REFRESH_INTERVAL) {
    p.displayZoneText(0, displayText, PA_CENTER, 50, 5000, PA_SCROLL_LEFT, PA_NO_EFFECT);
    p.displayZoneText(1, clockText, PA_CENTER, 100, 0, PA_NO_EFFECT, PA_NO_EFFECT);
    Serial.print("ESP32: Clock refreshed - "); Serial.println(clockText);
...
void updateClock() {
  ...
  int hours = (totalSeconds / 3600) % 16;
  ...
  p.displayZoneText(1, clockText, PA_CENTER, 100, 0, PA_NO_EFFECT, PA_NO_EFFECT);
  Serial.print("ESP32: Clock updated - "); Serial.println(clockText);
}